...
std::vector<Node*> AStar::FindPath(Node* startNode, Node* goalNode, std::vector<std::vector<int>>& grid)
{
    ...
    while (!openList.empty())
    {
        ...
        for (const Direction& direction : directions)
        {
            ...
            // 이웃 노드가 열린 리스트에 있으면 더 좋은 비욜일 때만 처리
            if (openListNode)
            {
                // 비용 확인.
